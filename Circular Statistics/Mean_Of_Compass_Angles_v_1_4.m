function [ final_angle_compass_degrees ] = Mean_Of_Compass_Angles_v_1_4( degrees_angles, draw_results )
% version 1.2 deletes NaN inputs
% v 1.3 has better visualization with the plot
% on v.1.4 the command: Mean_Of_Compass_Angles_v_1_4( [0 180], 1 ) should return 'NaN'


% EXAMPLE:
% Mean_Of_Compass_Angles_v_1_4( [0 90 270 350], 0 )
% Mean_Of_Compass_Angles_v_1_4( [0 90 30 200 190 0 0 ], 1 )
% Mean_Of_Compass_Angles_v_1_4( [0 180], 1 ) %megali aporia, ti na dwsei auto
% arage ???
% Zah's Book 5th editoin page 613 % WORKING !!!!
% Mean_Of_Compass_Angles_v_1_4( [45 55 81 96 110 117 132 154 ], 1 ) 

% % Mean_Of_Compass_Angles_v_1_4( [45 55 81 96 110 117 132 NaN ], 1 )

% Mean_Of_Compass_Angles_v_1_4( [45 55 81 96 110 117 132 NaN ],1)
% Mean_Of_Compass_Angles_v_1_4( [45 55 81 96 110 117 132 NaN ])
...
if min(size(degrees_angles))~=1
   error('Something is going wrong with the degrees given') 
end
...
% v.1.4 modification
if  abs(complex_sum)<1e-15 %complex_sum==0
   final_angle_compass_degrees = NaN;
   return
else
